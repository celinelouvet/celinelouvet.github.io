...
const decisions = [
  { text: 'Autorisé', shown: false },
  { text: 'Refusé', shown: false },
];
const executions = [
  ...
  >
    <HStack gap="brand.200" width="100%" height="100%" alignItems="flex-start">
      <Stack flex="2" gap="brand.100">
        ...
        <Stack gap="brand.100" marginLeft="brand.100" opacity={0}>
          <Text>Réponse binaire</Text>
          <ProgressiveList items={decisions} marginBottom={0} />
        </Stack>
      </Stack>
      <Box flex="1" alignSelf="center" textAlign="center" opacity={0}>
        <Icon as={TbArrowBigRightFilled} />
      </Box>

      <Stack flex="2" gap="brand.100" opacity={0}>
...
export const note = (
  <SlideNote>
    <Text>Une décision sera toujours une réponse binaire.</Text>
    <Text>Soit l’acteur est autorisé à faire l’action.</Text>
  </SlideNote>
);
